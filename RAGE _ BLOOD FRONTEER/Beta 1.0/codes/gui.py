...
class WidgetBar:
	def __init__(self, x, y, width, height, max_value, icon_id, direction, color, current_value=0):
		"""
		WidgetBar의 init 함수
		:param x: WidgetBar의 x 좌표
		:param y: WidgetBar의 y 좌표
		:param width: WidgetBar의 너비
		:param height: WidgetBar의 높이
		:param max_value:
		:param icon_id:
		:param direction:
		:param color:
...
if __name__ == "__main__":
	...
	a, b, c = 10, 10, 3
	S = StatsBar(window)
	widget_bar = WidgetBar(50, 50, 300, 50, 1000000, "path_to_icon_image.png", "right", 723982) # 아이콘 경로를 설정해야 합니다.
	while True:
		# if k == 360: k = 0
		window.fill((0))
